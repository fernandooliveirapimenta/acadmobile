...
.factory('loadingService', function($ionicLoading){
	loadingService = {};
	loadingService.open = function(){$ionicLoading.show({	content: 'Loading',	animation: 'fade-in', showBackdrop: true,	maxWidth: 200,showDelay: 0	});	};
...
.factory('noticiaService', function(servicoAcad){
	...
	noticiaService.url = function(){
		...
		if(user !== null)  urlnoticia +='/'+user.IdUsuario+'?perfil='+user.PerfilUsuario;
		  return urlnoticia;
	};
